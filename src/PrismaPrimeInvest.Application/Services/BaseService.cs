    ...
    where TFilter : FilterBase, new()
{
    ...
    {
        if (!string.IsNullOrEmpty(filter.Search) && filter.SearchFields?.Length > 0)
            query = query.ApplyTextSearch(filter.Search, filter.SearchFields);
        ...
        if (!string.IsNullOrEmpty(filter.SortBy))
            query = query.OrderByDynamic(filter.SortBy, filter.SortDirection ?? "asc");
    ...
    public virtual async Task<PagedResult<TDto>> GetAllAsync(TFilter filter)
    {
        ...
        return new PagedResult<TDto>(_mapper.Map<List<TDto>>(items), totalItems, filter.Page, filter.PageSize ?? totalItems);
    }
